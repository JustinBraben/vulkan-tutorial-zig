...
const HelloTriangleApplication = struct {
    ...
    fn createSyncObjects(self: *Self) !void {
        self.image_available_semaphore = try self.device.createSemaphore(&.{ .flags = .{} }, null);
        self.render_finished_semaphore = try self.device.createSemaphore(&.{ .flags = .{} }, null);
        self.in_flight_fence = try self.device.createFence(&.{ .flags = .{ .signaled_bit = true } }, null);
    }
    ...
    fn drawFrame(self: *Self) !void {
        ...
        const result = try self.device.acquireNextImageKHR(self.swap_chain, std.math.maxInt(u64), self.image_available_semaphore, .null_handle);
        ...
        _ = try self.device.queueSubmit(self.graphics_queue, 1, &[_]vk.SubmitInfo{submit_info}, self.in_flight_fence);
