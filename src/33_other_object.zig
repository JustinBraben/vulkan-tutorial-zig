...
const HelloTriangleApplication = struct {
    ...
    camera: Camera = .{},
    mouse_last_x: f64 = 400,
    mouse_last_y: f64 = 300,
    ...
    fn initVulkan(self: *Self) !void {
        ...
        // In initVulkan, after creating swap chain:
    ...
    fn handleInput(self: *Self, delta_time: f32) !void {
        ...
        if (c.glfwGetKey(self.window, c.GLFW_KEY_RIGHT) == c.GLFW_PRESS) {
            self.camera.rotation = self.camera.rotation.add(Vec3.new(0, speed * 50.0, 0));
        }
        if (c.glfwGetKey(self.window, c.GLFW_KEY_UP) == c.GLFW_PRESS) {
    ...
    fn cleanupSwapChain(self: *Self) void {
        if (self.depth_image_view != .null_handle) self.device.destroyImageView(self.depth_image_view, null);
        ...
        if (self.color_image_view != .null_handle) self.device.destroyImageView(self.color_image_view, null);
        if (self.color_image != .null_handle) self.device.destroyImage(self.color_image, null);
        if (self.color_image_memory != .null_handle) self.device.freeMemory(self.color_image_memory, null);
    ...
    fn createTextureImage(self: *Self) !void {
        ...
        self.mip_levels = std.math.log2(@max(@as(u32, @intCast(tex_width)), @as(u32, @intCast(tex_height)))) + 1;
        ...
        try self.generateMipmaps(self.texture_image, .r8g8b8a8_srgb, @as(u32, @intCast(tex_width)), @intCast(tex_height), self.mip_levels);
    }
    ...
    fn generateMipmaps(self: *Self, image: vk.Image, image_format: vk.Format, tex_width: u32, tex_height: u32, mip_levels: u32) !void {
        ...
        while (i < mip_levels) : (i += 1) {
            ...
            self.device.cmdPipelineBarrier(command_buffer, .{ .transfer_bit = true }, .{ .transfer_bit = true }, .{}, 0, undefined, 0, undefined, barrier.len, &barrier);
            ...
            const blit = [_]vk.ImageBlit{.{
                ...
                .dst_offsets = [2]vk.Offset3D{
                    .{ .x = 0, .y = 0, .z = 0 },
                    .{ .x = @intCast(if (mip_width > 1) mip_width / 2 else 1), .y = @intCast(if (mip_height > 1) mip_height / 2 else 1), .z = 1 },
                },
                .dst_subresource = .{
                    .aspect_mask = .{ .color_bit = true },
                    .mip_level = i,
            ...
            self.device.cmdPipelineBarrier(command_buffer, .{ .transfer_bit = true }, .{ .fragment_shader_bit = true }, .{}, 0, undefined, 0, undefined, barrier.len, &barrier);
    ...
    fn getMaxUsableSampleCount(self: *Self) vk.SampleCountFlags {
        const physical_device_properties = self.instance.getPhysicalDeviceProperties(self.physical_device);
    ...
    fn recordCommandBuffer(self: *Self, command_buffer: vk.CommandBuffer, image_index: u32) !void {
        ...
        self.device.cmdBeginRenderPass(command_buffer, &render_pass_info, .@"inline");
        {
            ...
            self.device.cmdBindIndexBuffer(command_buffer, self.index_buffer, 0, vk.IndexType.uint32);
            ...
            self.device.cmdDrawIndexed(command_buffer, @intCast(self.indices.items.len), 1, 0, 0, 0);
        }
    ...
    fn updateUniformBuffer(self: *Self, current_image: u32) !void {
        var ubo = UniformBufferObject{
            .model = za.Mat4.identity().rotate(-90.0, za.Vec3.new(1.0, 0.0, 0.0)),
            .view = self.camera.matrices.view,
            .proj = self.camera.matrices.perspective,
        };
...
const Camera = struct {
    ...
    rotation: za.Vec3 = za.Vec3.zero(),
    position: za.Vec3 = za.Vec3.zero(),
    view_pos: za.Vec4 = za.Vec4.zero(),
    ...
    pub fn updateViewMatrix(self: *@This()) void {
        // For FPS camera, we need to invert the transformations
        if (self.camera_type == .firstperson) {
            // For FPS camera, we build the view matrix differently
            // First, calculate the camera's forward direction based on yaw and pitch
    ...
    pub fn setPerspective(self: *@This(), fov: f32, aspect: f32, znear: f32, zfar: f32) void {
        ...
        self.z_near = znear;
        self.z_far = zfar;
        self.matrices.perspective = za.perspective(fov, aspect, znear, zfar);
        // Always flip Y for Vulkan
        self.matrices.perspective.data[1][1] *= -1.0;
        self.updated = true;
    }
    ...
    pub fn update(self: *@This(), deltaTime: f32) void {
        ...
        if (self.camera_type == .firstperson) {
            if (self.moving()) {
                ...
                const world_up = Vec3.new(0.0, 1.0, 0.0);
                const cam_right = cam_front.cross(world_up).norm();
                const move_speed: f32 = deltaTime * self.movement_speed;
