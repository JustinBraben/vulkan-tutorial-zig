...
const vert_spv align(@alignOf(u32)) = resources.shaders.vert_26.*;
const frag_spv align(@alignOf(u32)) = resources.shaders.frag_26.*;
...
const HelloTriangleApplication = struct {
    ...
    device_properties: vk.PhysicalDeviceProperties2 = undefined,
    ...
    descriptor_buffer_address: vk.DeviceAddress = 0,
    descriptor_buffer_properties: vk.PhysicalDeviceDescriptorBufferPropertiesEXT = undefined,
    descriptor_size_ubo: vk.DeviceSize = 0,
    descriptor_size_sampler: vk.DeviceSize = 0,
    ...
    pub fn deinit(self: *Self) void {
        ...
        if (self.uniform_buffers != null) {
            for (self.uniform_buffers.?) |uniform_buffer| {
                self.device.destroyBuffer(uniform_buffer, null);
            }
        ...
        if (self.uniform_buffers_memory != null) {
            for (self.uniform_buffers_memory.?) |uniform_buffer_memory| {
                self.device.freeMemory(uniform_buffer_memory, null);
            }
        ...
        if (self.descriptor_buffer_memory != .null_handle) self.device.freeMemory(self.descriptor_buffer_memory, null);

        if (self.texture_sampler != .null_handle) self.device.destroySampler(self.texture_sampler, null);
        if (self.texture_image_view != .null_handle) self.device.destroyImageView(self.texture_image_view, null);

        if (self.texture_image != .null_handle) self.device.destroyImage(self.texture_image, null);
        if (self.texture_image_memory != .null_handle) self.device.freeMemory(self.texture_image_memory, null);

        if (self.descriptor_set_layout != .null_handle) self.device.destroyDescriptorSetLayout(self.descriptor_set_layout, null);
    ...
    fn createLogicalDevice(self: *Self) !void {
        ...
        // Enable descriptor indexing features (required)
    ...
    fn createTextureImage(self: *Self) !void {
        ...
        const pixels = c.stbi_load("C:/Users/Justin/source/forks/vulkan-tutorial-zig/resources/texture.jpg", &tex_width, &tex_height, &channels, c.STBI_rgb_alpha);
        defer c.stbi_image_free(pixels);
        if (pixels == null) {
            return error.ImageLoadFailure;
        }

        const image_size: vk.DeviceSize = @as(u64, @intCast(tex_width)) * @as(u64, @intCast(tex_height)) * 4;
        ...
        const data = try self.device.mapMemory(staging_buffer_memory, 0, image_size, .{});
        std.mem.copyForwards(u8, @as([*]u8, @ptrCast(data.?))[0..image_size], pixels[0..image_size]);
    ...
    fn createTextureSampler(self: *Self) !void {
        ...
        self.texture_sampler = try self.device.createSampler(&sampler_info, null);
    }
    ...
    fn createImageView(self: *Self, image: vk.Image, format: vk.Format) !vk.ImageView {
        ...
        return try self.device.createImageView(&view_info, null);
    }
    ...
    fn createDescriptorBuffer(self: *Self) !void {
        // Calculate total size needed for all descriptor sets
        const descriptor_set_size = self.device.getDescriptorSetLayoutSizeEXT(self.descriptor_set_layout);
        ...
        // Create descriptor buffer with appropriate usage flags
        ...
        self.descriptor_buffer = try self.device.createBuffer(&buffer_info, null);
        std.debug.print("descriptor_buffer: {any}\n", .{self.descriptor_buffer});
        
        // Allocate memory for descriptor buffer
        const mem_requirements = self.device.getBufferMemoryRequirements(self.descriptor_buffer);
        std.debug.print("mem_requirements: {any}\n", .{mem_requirements});

        var alloc_flags_info = vk.MemoryAllocateFlagsInfo{
        ...
        self.descriptor_buffer_memory = try self.device.allocateMemory(&alloc_info, null);
        try self.device.bindBufferMemory(self.descriptor_buffer, self.descriptor_buffer_memory, 0);
        
        // Get buffer device address
        const addr_info = vk.BufferDeviceAddressInfo{
            .buffer = self.descriptor_buffer,
        };
        self.descriptor_buffer_address = self.device.getBufferDeviceAddress(&addr_info);
        ...
        .{i, ubo_offset, sampler_offset});
            ...
            // Use the uniform buffer descriptor size from properties
            ...
            // Use the combined image sampler descriptor size from properties
            self.device.getDescriptorEXT(
    ...
    fn createBuffer(self: *Self, size: vk.DeviceSize, usage: vk.BufferUsageFlags, properties: vk.MemoryPropertyFlags, buffer: *vk.Buffer, buffer_memory: *vk.DeviceMemory) !void {
        ...
        buffer_memory.* = try self.device.allocateMemory(&alloc_info, null);
    ...
    fn recordCommandBuffer(self: *Self, command_buffer: vk.CommandBuffer, image_index: u32) !void {
        ...
        //     self.device.cmdBindDescriptorSets(command_buffer, .graphics, self.pipeline_layout, 0, 1, @ptrCast(&self.descriptor_sets.?[self.current_frame]), 0, undefined);
        ...
        self.device.cmdBeginRenderPass(command_buffer, &render_pass_info, .@"inline");
        {
            ...
            const set_layout_size = self.device.getDescriptorSetLayoutSizeEXT(self.descriptor_set_layout);
            const address_offset = self.descriptor_buffer_address + (self.current_frame * set_layout_size);

            // Bind descriptor buffer instead of descriptor sets
            const descriptor_buffer_binding_info = [_]vk.DescriptorBufferBindingInfoEXT{
                .{
            ...
            // std.debug.print("descriptor_buffer_binding_info.len: {d}\n", .{descriptor_buffer_binding_info.len});
            // self.descriptor_buffer
            self.device.cmdBindDescriptorBuffersEXT(command_buffer, descriptor_buffer_binding_info.len, &descriptor_buffer_binding_info);
            
            // Set descriptor buffer offsets
            const buffer_indices = [_]u32{0};
            const descriptor_offsets = [_]vk.DeviceSize{0};
            // const descriptor_set_size = self.device.getDescriptorSetLayoutSizeEXT(self.descriptor_set_layout);
            
            self.device.cmdSetDescriptorBufferOffsetsEXT(
                command_buffer,
                .graphics,
            ...
            self.device.cmdDrawIndexed(command_buffer, @intCast(indices_input.len), 1, 0, 0, 0);
        }
        self.device.cmdEndRenderPass(command_buffer);
    ...
    fn updateUniformBuffer(self: *Self, current_image: u32) !void {
        const time: f32 = (@as(f32, @floatFromInt((try std.time.Instant.now()).since(self.start_time))) / @as(f32, @floatFromInt(std.time.ns_per_s)));
        ...
        var ubo = UniformBufferObject{
            ...
            .proj = za.perspective(45.0, @as(f32, @floatFromInt(self.swap_chain_extent.width)) / @as(f32, @floatFromInt(self.swap_chain_extent.height)), 0.1, 10),
        };
        ubo.proj.data[1][1] *= -1;

        const data = try self.device.mapMemory(self.uniform_buffers_memory.?[current_image], 0, @sizeOf(UniformBufferObject), .{});
        std.mem.copyForwards(u8, @as([*]u8, @ptrCast(data.?))[0..@sizeOf(UniformBufferObject)], std.mem.asBytes(&ubo));
        self.device.unmapMemory(self.uniform_buffers_memory.?[current_image]);
    }
    ...
    fn checkDeviceExtensionSupport(self: *Self, device: vk.PhysicalDevice) !bool {
        ...
        // for (available_extensions) |avail_ext| {
        //     std.debug.print("Available device extension: {s}\n", .{avail_ext.extension_name});
        // }
    ...
    fn getRequiredExtensions(allocator: Allocator) !std.ArrayListAligned([*:0]const u8, null) {
        ...
        for (extension_names.items) |ext_name| {
            std.debug.print("Extension added: {s}\n", .{ext_name});
        }
...
fn hexDump(ptr: [*]const u8, len: usize) void {
    var i: usize = 0;
    while (i < len) : (i += 16) {
        std.debug.print("{x:08}  ", .{i});
        var j: usize = 0;
        while (j < 16 and (i + j) < len) : (j += 1) {
            std.debug.print("{x:02} ", .{ptr[i + j]});
        }
        while (j < 16) : (j += 1) {
            std.debug.print("   ", .{});
        }
        ...
        j = 0;
        while (j < 16 and (i + j) < len) : (j += 1) {
            const char_1 = ptr[i + j];
            const char_2 = if (char_1 >= 32 and char_1 <= 126) char_1 else '.';
            std.debug.print("{c}", .{char_2});
        }
        std.debug.print("\n", .{});
    }
}
...
pub fn main() void {
    ...
    var app = HelloTriangleApplication.init(gpa) catch |err| {
